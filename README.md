...
- 🤖 **多模型支持**：集成 DeepSeek、GLM、Qwen-VL 等多种顶级 AI 模型
...
│       │   ├── mcp/               # 模型调用协议
│       │   ├── skills/            # 技能模块
│       │   │   ├── textSkill.ts     # 文本对话（DeepSeek）
│       │   │   ├── multimodalSkill.ts # 多模态（Qwen-VL）
...
所有 API 调用都封装在 `packages/frontend/src/api/ai.ts` 中，通过 `sendChatRequest` 函数进行。

API 端点：`https://api.i-tool-chat.store`
...
- `VITE_API_URL`：API 服务器地址，默认为 `https://api.i-tool-chat.store`

### Worker

Worker 需要配置以下环境变量（至少配置一个）：

- `DEEPSEEK_API_KEY`：DeepSeek API 密钥（推荐，用于文本对话）
- `QWEN_API_KEY`：通义千问 API 密钥（可选，用于多模态图片处理）
- `GLM_API_KEY`：智谱 GLM API 密钥（可选，用于 GLM 模型）
...
| 技能名称 | 文件 | 功能 | 支持模型 |
|---------|------|------|---------|
| textSkill | textSkill.ts | 纯文本对话 | DeepSeek |
| glmSkill | glmSkill.ts | GLM 模型对话 | GLM-4.6 |
| multimodalSkill | multimodalSkill.ts | 图片+文本处理 | Qwen-VL |
| fileSkill | fileSkill.ts | 文件上传处理 | DeepSeek/GLM |

技能路由规则：
- 有文件上传 → fileSkill
- 有图片上传 → multimodalSkill
- 纯文本 → textSkill 或 glmSkill
