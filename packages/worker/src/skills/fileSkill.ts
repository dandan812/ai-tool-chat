 ...
 */
import type { Skill, SkillInput, SkillContext, SkillStreamChunk, Message, FileData } from '../types';
import { textSkill } from './textSkill';
import { logger } from '../utils/logger';
import { estimateTokens, splitByTokens, addContextToChunks } from '../utils/chunker';
 ...
 * 最终答案生成提示词模板
 */
const FINAL_ANSWER_PROMPT = (summaries: string, userQuestion: string) => `我有一个大文件，已经分块分析。以下是各分块的摘要：
...
async function* processLargeFiles(
  ...
  const finalMessages: Message[] = [
    {
      role: 'system',
      content: '你是一个文件分析助手。用户上传了一个大文件，已经分块提取了摘要。请基于这些摘要回答用户问题。'
    },
    {
      role: 'user',
      content: finalPrompt
...
async function extractSingleChunkSummary(
  ...
  const messages: Message[] = [
    {
      role: 'system',
      content: '你是一个文件分析助手。用户上传了一个大文件，已经分块。请提取当前分块的核心内容和结构。'
    },
    {
      role: 'user',
  ...
  // 收集流式响应
  let summary = '';
  for await (const chunk of textSkill.execute(textInput, context)) {
  ...
  logger.info(`Summary extracted for chunk ${chunkIndex}`, { summaryLength: summary.length });
  return summary;
}
